...
import 'package:ephysicsapp/screens/Admin/appUsageStatistics.dart';
...
class _AdminStatisticsState extends State<AdminStatistics> {
  ...
  Future<void> getUserStatsAndCount() async {
    ...
    try {
      DataSnapshot snapshot = await dbRef.once().then((event) => event.snapshot);
      ...
      if (snapshot.exists) {
        ...
        snapshot.children.forEach((userSnapshot) {
          ...
          if (userData.containsKey('pdfsViewed')) {
            totalPdfsViewed += (userData['pdfsViewed'] as num).toInt(); // Cast to num and then to int
          }
          if (userData.containsKey('videosViewed')) {
            totalVideosViewed += (userData['videosViewed'] as num).toInt(); // Cast to num and then to int
          }
      ...
      body: isLoading
        ...
        child: Padding(
          ...
          child: GridView.count(
            ...
            childAspectRatio: 1.2, // Aspect ratio to control the height/width ratio of the boxes
            children: [
              _buildStatBox(Icons.people, 'Total Users', userCount.toString(), Colors.blue, context, 'TotalUsersPage'),
              _buildStatBox(Icons.picture_as_pdf, 'PDFs Viewed', totalPdfsViewed.toString(), Colors.green, context, 'PdfsViewedPage'),
              _buildStatBox(Icons.video_library, 'Videos Viewed', totalVideosViewed.toString(), Colors.orange, context, 'VideosViewedPage'),
              _buildStatBox(Icons.school, 'Unique Colleges', uniqueCollegesCount.toString(), Colors.red, context, 'UniqueCollegesPage'),
            ],
          ),
        ),
      ),
  ...
  Widget _buildStatBox(IconData icon, String label, String value, Color color, BuildContext context, String route) {
    bool isTapped = false;
    ...
    return GestureDetector(
      ...
      onTap: () {
        Navigator.push(context, MaterialPageRoute(builder: (context) => AppUsageStats()));
      },
      child: AnimatedContainer(
        duration: const Duration(milliseconds: 150),
        transform: Matrix4.identity()..scale(isTapped ? 0.95 : 1.0), // Scale on touch
